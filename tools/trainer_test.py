...
import _init_path
from object_detection.tools import trainer
	...
	def preprocess(self, inputs):
		...
		Returns:
			...
			true_image_shapes: int32 tensor of shape [batch, 3] where each row is
				...
				with zeros.
		"""
		#true_image_shapes = [inputs.shape[:-1].as_list() for _ in range(inputs.shape[-1])]
		true_image_shapes = [inputs.shape[1:].as_list() for _ in range(inputs.shape[0])]
		return tf.image.resize_images(inputs, size=[28, 28]), true_image_shapes
...
class TrainerTest(tf.test.TestCase):
	...
	def test_configure_trainer_with_multiclass_scores_and_train_two_steps(self):
		print('\n========================================================================')
		print('test_configure_trainer_with_multiclass_scores_and_train_two_steps')
...
if __name__ == '__main__':
	...
	TrainerTest().test_configure_trainer_and_train_two_steps()
	TrainerTest().test_configure_trainer_with_multiclass_scores_and_train_two_steps()
