...
class SSDMetaArch(model.DetectionModel):
	...
	def __init__(self,
		...
		if encode_background_as_zeros:
			unmatched_cls_target = tf.constant((self.num_classes + 1) * [0], tf.float32)
	...
	def _compute_clip_window(self, preprocessed_images, true_image_shapes):
		...
		true_heights, true_widths, _ = tf.unstack(tf.to_float(true_image_shapes), axis=1)
	...
	def _get_feature_map_spatial_dims(self, feature_maps):
		...
		"""
		feature_map_shapes = [shape_utils.combined_static_and_dynamic_shape(
