...
class PrefetcherTest(tf.test.TestCase):
	...
	def test_prefetch_tensors_with_fully_defined_shapes(self):
		print('\n===============================================================')
		print('test_prefetch_tensors_with_fully_defined_shapes')
		...
		with self.test_session() as sess:
			...
			image = tf.random_normal([batch_size, image_size, image_size, 3],
									 dtype=tf.float32, name='images')
			label = tf.random_uniform([batch_size, 1], minval=0, maxval=10,
			...
			with slim.queues.QueueRunners(sess):
				for _ in range(num_batches):
					...
					for key in results.keys():
						print('  {}: {}'.format(key, results[key].shape))
	...
	def test_prefetch_tensors_with_partially_defined_shapes(self):
		print('\n======================================================================')
		print('test_prefetch_tensors_with_partially_defined_shapes')
		...
		with self.test_session() as sess:
			...
			image = tf.random_normal([batch_size, tf.Variable(image_size), tf.Variable(image_size), 3],
									 dtype=tf.float32,
									 name='image')
			image.set_shape([batch_size, None, None, 3])
			label = tf.random_uniform([batch_size, tf.Variable(1)], 0, 10, 
...
if __name__ == '__main__':
	...
	PrefetcherTest().test_prefetch_tensors_with_fully_defined_shapes()
	PrefetcherTest().test_prefetch_tensors_with_partially_defined_shapes()
