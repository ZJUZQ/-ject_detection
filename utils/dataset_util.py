...
def read_dataset(file_read_func, decode_func, input_files, config):
  ...
  filenames = tf.concat([tf.matching_files(pattern) for pattern in input_files],
                        0)
  filename_dataset = tf.data.Dataset.from_tensor_slices(tensors=filenames)
  if config.shuffle:
    filename_dataset = filename_dataset.shuffle(
        buffer_size=config.filenames_shuffle_buffer_size)

  elif config.num_readers > 1:
    tf.logging.warning('`shuffle` is false, but the input data stream is '
                       'still slightly shuffled since `num_readers` > 1.')

  filename_dataset = filename_dataset.repeat(count=config.num_epochs or None)

  records_dataset = filename_dataset.apply(
      tf.contrib.data.parallel_interleave(
          map_func=file_read_func, cycle_length=config.num_readers,
