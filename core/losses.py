...
class WeightedSmoothL1LocalizationLoss(Loss):
	...
	0.5 delta^2 + delta * (|x|-delta) otherwise, where x is the difference between
...
class WeightedSigmoidClassificationLoss(Loss):
	...
	def _compute_loss(self,
		...
		Args:
			prediction_tensor: A float tensor of shape [batch_size, num_anchors, num_classes] 
								representing the predicted logits for each class
			target_tensor: A float tensor of shape [batch_size, num_anchors, num_classes] 
