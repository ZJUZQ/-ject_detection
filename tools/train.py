...
flags = tf.app.flags
flags.DEFINE_string('gpus_train', '(0,)', 'gpus used to train')
...
def set_visible_devices(sys_device_ids):
	"""
	Set some GPUs to be visible 
	Args:
...
def main(_):
	gpus_train = eval(str(FLAGS.gpus_train))
	set_visible_devices(gpus_train)
	...
	if FLAGS.pipeline_config_path:
		...
		if FLAGS.task == 0:
			tf.gfile.Copy(oldpath=FLAGS.pipeline_config_path,
						  newpath=os.path.join(FLAGS.train_dir, 'pipeline.config'),
						  overwrite=True)
	else:
