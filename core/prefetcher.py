...
def prefetch(tensor_dict, capacity):
	...
	A PaddingFIFOQueue may contain components with dynamic shape, while also supporting dequeue_many
	"""
	prefetch_queue = tf.PaddingFIFOQueue(capacity, dtypes=dtypes,
												   shapes=shapes,
												   names=names,
												   name='prefetch_queue')
	enqueue_op = prefetch_queue.enqueue(vals=tensor_dict)

	# tf.train.QueueRunner: Holds a list of enqueue operations for a queue, each to be run in a thread
	tf.train.queue_runner.add_queue_runner(
			qr=tf.train.queue_runner.QueueRunner(queue=prefetch_queue, 
