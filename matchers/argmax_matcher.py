...
class ArgMaxMatcher(matcher.Matcher):
	...
	def _match(self, similarity_matrix):
		...
		def _match_when_rows_are_non_empty(verbose=False):
			...
			if self._force_match_for_each_row:
				# First ensures that each row is matched to at least one column (which is not 
				# guaranteed otherwise if the matched_threshold is high)
				...
				if verbose:
					force_match_column_ids = tf.Print(force_match_column_ids, 
													  ['force_match_column_ids: ', force_match_column_ids])
				
				force_match_column_indicators = tf.one_hot(force_match_column_ids, depth=similarity_matrix_shape[1])
				if verbose:
					force_match_column_indicators = tf.Print(force_match_column_indicators, 
															 ['force_match_column_indicators: ', force_match_column_indicators])

				force_match_row_ids = tf.argmax(force_match_column_indicators, 0, output_type=tf.int32)
				if verbose:
					force_match_row_ids = tf.Print(force_match_row_ids, 
												   ['force_match_row_ids: ', force_match_row_ids])
